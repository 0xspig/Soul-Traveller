...
vec2 CRTCurveUV(vec2 uv) {
	if(show_curvature) {
		uv = uv * 2.0 - 1.0;
		vec2 offset = abs(uv.yx) / vec2(20., 1.0);
...
void fragment() {
	...
	float cLength = length(UV - center);
	vec2 uv_offset = vec2(cLength * 10. * cos(cLength * 40. + TIME * 20.) * .02) * intensity;
	
	vec2 screen_crtUV = CRTCurveUV(SCREEN_UV) + uv_offset;
...
//	vec2 uv_offset = vec2(cLength * 10. * cos(cLength * 40. + TIME * 20.) * .02) * intensity;
//
//	vec4 tex = textureLod(SCREEN_TEXTURE, SCREEN_UV + uv_offset, 0.0);
